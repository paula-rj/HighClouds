...
class Feedbacks:
    ...
    def res_feedbacks(area, K, sst_anom, which):
        ...
        K_R = K - K_prima - K_prima

        R_res = (K_R * cc_ast).sum(["press", "opt"])

        feed_res = stats.linregress(sst_anom, R_res.mean("lat")).slope
